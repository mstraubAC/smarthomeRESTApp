...
type queryParametersGetElectricEnergyFlowDaily struct {
...
func (h *handler) getElectricEnergyFlowDaily(c *gin.Context) {
	ctx := context.Background()

	// parameter validation
	params := queryParametersGetElectricEnergyFlowDaily{}
	...
	err = pgxscan.Select(ctx, sqlConn, &locations,
		`SELECT 
			logdate
			,electricgridconsumevalue AS electricconsumevalue
			,electricGridFeedInValue AS electricgridfeedinvalue
			,electricPvGenerationValue AS electricgridpvgenerationvalue
			,electricHeatingConsumeValue AS electricheatingconsumevalue
